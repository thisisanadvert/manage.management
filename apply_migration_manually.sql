-- Manual Migration: Fix Issue Timeline RLS Policy
-- Run this in Supabase SQL Editor to fix the issue creation problem

-- First, check if the policies already exist and drop them if they do
DROP POLICY IF EXISTS "Users can create timeline entries for issues in their buildings" ON issue_timeline;
DROP POLICY IF EXISTS "Users can update timeline entries for issues in their buildings" ON issue_timeline;
